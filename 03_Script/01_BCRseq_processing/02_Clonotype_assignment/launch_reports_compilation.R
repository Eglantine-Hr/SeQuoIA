...
if(DONOR_COLUMN == "None" | length(na.omit(scRNAseq_metadata_path)) < 1 ){  
	bcr.df = bcr.df.all
	other_donor_barcodes = "no donor" #metadata_df[ which( metadata_df[ DONOR_COLUMN ]  !=  current_donor  ) , ]$barcode 
	...
	for(  current_donor in all_donors     ){
		...
		if("barcode" %in% names(metadata_df) == F & "cell_id" %in% names(metadata_df) == T ){ metadata_df$barcode = metadata_df$cell_id} 
	...
	bcr.df.clono_list = as.data.frame( do.call("rbind", bcr.df.clono_list) ) %>% dplyr::mutate( isInMetadata = ifelse(cell_id %in% metadata_df$barcode , T, F )  ) %>%
