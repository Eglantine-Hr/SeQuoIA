...
#   	#echo "$NCAabsolute"
   	...
   	if test -f "Heavy_Absolute_Germline.txt"; then 
		...
		#echo ${NCAabsolute}
   		PGEN=$(igor-compute_pgen ${organism} heavy_naive ${NCAabsolute:1:-1}) #computing probas, we remove quotation marks    heavy_naive
   	 	PGEN=${PGEN#*$'\n'} #${PGEN#*txt}#https://stackoverflow.com/questions/428109/extract-substring-in-bash           ##PGEN=${PGEN#*$'\n'} #pk marche?
   	 	#echo ${PGEN} 
   	 	echo "${CloneName}, Heavy, ${PGEN}" >> ${currentdir}/${outputPath}/$3_Pgen_VDJ_summary.txt #adding data to summary table ##HOW TO IMRPOVE THIS STEP? 
		#break 
   	fi
